							...
							while (i < OPedLanza.Length && tActual >= OPedLanza[i].Item2)
							{
								...
								if (pos > -1)
								{
									if(OPedLanza[i].Item2 != -1)
										plan.insertarPedido(pedido.IdPedido, new Tuple<double, int>((pedido.Plazo / (pedido.NPiezas * tiempos[pos])) *(pedido.Prioridad + 1) , pedido.NPiezas * tiempos[pos])); 
									// En la tupla, el primer campo se aplica la heurista para planificar los pedidos
									// (pedido.Plazo / (pedido.NPiezas * tiempos[pos])) *(pedido.Prioridad + 1) || (pedido.Plazo / (pedido.NPiezas * tiempos[pos])) 
									// || pedido.Plazo || (pedido.NPiezas * tiempos[pos])
									else
										plan.insertarPedido(pedido.IdPedido, new Tuple<double, int>(-1,-1));
								}
								i++;
							}
							plan.setId(i);
